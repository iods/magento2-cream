...
class EavAtrributeUpdateHelper extends AbstractHelper
{
    ...
    protected function _saveAttributeValue($object, $attribute, $value)
    {
        ...
        if ($this->entityType == \Magento\Catalog\Model\Product::ENTITY) {
            if ($attribute->getIsGlobal() == \Magento\Catalog\Model\ResourceModel\Eav\Attribute::SCOPE_GLOBAL) {
                /**
                 * Update attribute value for store
                 */
                $this->_attributeValuesToSave[$table][] = $bind;
            } elseif ($attribute->getIsGlobal() == \Magento\Catalog\Model\ResourceModel\Eav\Attribute::SCOPE_WEBSITE && $storeId != $this->getDefaultStoreId()) {
                /**
...
Owner
Author
mageseller commented on Dec 5, 2020
Use as below :
...
For Category:
    ...
    $storeId = 0;
    $this->updateAttributeHelper->updateCategoryAttributes($categoryIds,$attributes,$storeId);
    For customer :
        $this->updateAttributeHelper->updateCustomerAttributes($customerIds,$attributes,$storeId);

        For customer address :
            $this->updateAttributeHelper->updateCustomerAddressAttributes($customerAddressIds,$attributes,$storeId);

            For Product :
                $this->updateAttributeHelper->updateProductAttributes($productIds,$attributes,$storeId);
